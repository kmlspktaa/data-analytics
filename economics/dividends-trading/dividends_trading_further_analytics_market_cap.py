...
total_dividends = total_dividends.join(df.select("Symbol", "Market Capitalization", "Year"), ["Symbol", "Year"])
...
total_dividends = total_dividends.select("Symbol", "total_yearly_dividends", format_number(col("Market_Cap"), 0).alias("Market_Cap"), "Year")
...
# total_dividends.write.csv("total_dividends.csv", header=True, mode="overwrite")

# Convert PySpark DataFrame to Pandas DataFrame
pandas_df = total_dividends.toPandas()

# Save the Pandas DataFrame as an Excel file
pandas_df.to_excel("total_dividends.xlsx", index=False)
